...
>(({ className, items, defaultActiveIndex = 0, ...props }, ref) => {
  ...
  return (
    <ul
      className={cn(
        "w-full gap-2 grid h-[600px] md:h-[500px]",
    ...
    >
      {items.map((item, index) => (
        <li
          key={item.id}
          className="group relative cursor-pointer overflow-hidden rounded-2xl md:min-w-[80px] min-h-0 min-w-0"
        ...
        >
          ...
          <article className="absolute inset-0 z-10 flex flex-col justify-end gap-2 p-6 pointer-events-none">
            <h3 className="hidden origin-left rotate-90 text-xs font-light uppercase tracking-[0.2em] text-white/80 opacity-100 transition-all duration-300 ease-out md:block group-data-[active=true]:opacity-0 whitespace-nowrap">
              {item.title}
            </h3>
            ...
            <h3 className="text-2xl font-medium text-white opacity-0 transition-all duration-300 delay-150 ease-out group-data-[active=true]:opacity-100">
              {item.title}
            </h3>
