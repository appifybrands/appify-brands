...
const HoverExpand_002 = ({
  ...
  return (
    <motion.div
      ...
      className={cn("relative w-full h-full max-w-sm mx-auto px-2", className)}
    >
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      >
        <div className="flex w-full h-full flex-col items-center justify-center gap-2 py-4">
              ...
              <AnimatePresence>
                {activeImage === index && (
                  <motion.div
                    ...
                    exit={{ opacity: 0, y: 20 }}
                    className="absolute flex h-full w-full flex-col items-end justify-end px-4 pb-4 z-20"
                  >
                    <p className="text-left text-xs text-white/80 font-medium tracking-wider">
                      {image.code}
                    </p>
                  </motion.div>
                )}
